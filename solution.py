...
def handle_claim_with_fraud_check(claim_data: Dict) -> Dict:
    ...
    print("############ self.tools --->", type(self.tools))
    print("claim_data ---> ", claim_data)
    # self.tools.append(handle_claim_with_fraud_check)
    
    
    #######################
    handle_claim_with_fraud_check_status = self.fraud_detector.run(
            f"""
            Analyze claim {claim_data} for potential fraud.
            
            First, get details about the claim using get_claim_details tool.
            Then check for fraud indicators using handle_claim_with_fraud_check tool.
            Provide a detailed assessment of any potential fraud risks.
            """
        )
...
def demonstrate_fraud_detection():
    ...
    print("orchestrator.tools ---> ", orchestrator.tools)
